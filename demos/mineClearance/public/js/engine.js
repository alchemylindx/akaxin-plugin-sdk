...
Storage.prototype.getInt = function(key) {
    return parseInt(this.storage.data(this.path + key) || 0)
};
Storage.prototype.getStr = function(key) {
    return (this.storage.data(this.path + key) || "")
};
Storage.prototype.save = function(key, value) {
    this.storage.data(this.path + key, value)
};
Storage.prototype.getJSON = function(key) {
    var value = this.storage.get(this.path + key);
    return (value ? JSON.parse(value) : null)
};
Storage.prototype.saveJSON = function(key, value) {
    this.storage.data(this.path + key, JSON.stringify(value))
};
Storage.prototype.erase = function(key) {
    this.storage.removeData(this.path + key)
};
function Control() {
    CLICK_EVENT = "click";
